...
_STATIC_SWAGGER = Path(__file__).resolve().parent / "static" / "swagger-ui"

app = FastAPI(
    title="Vercel + FastAPI",
    description="Vercel + FastAPI",
    version="1.0.0",
    # Relative server URL so Swagger "Execute" uses the same host as the docs page
    # (avoids broken requests to http://0.0.0.0:5001 or mismatched localhost/IPv6).
    servers=[{"url": "/", "description": "Current host"}],
    root_path_in_servers=False,
    # Served locally in /docs below (avoids "Failed to fetch" when CDN scripts are blocked).
    docs_url=None,
)
...
<span class="keyword">def</span> <span class="function">read_root</span>():
    <span class="keyword">return</span> {<span class="string">"Python"</span>: <span class="string">"on Vercel"</span>}</code></pre>
            ...
            <div class="cards">
                ...
                <div class="card">
                    <h3>Model metrics</h3>
                    <p>View sample accuracy and related metrics from the API.</p>
                    <a href="/api/metrics">Get metrics →</a>
                </div>
